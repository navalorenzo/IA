...
%   Un mondo di nome W � rappresentato dai predicati
...
%  world(W, NCol, NRow) :
%  W � un mondo caricato in memoria dinamica con NCol colonne
...
%  navigable(W, P) :  P punto in cui l'agente pu� passare
%  MODO  (+,+) semidet
%
...
%  di caratteri  (vedi pi� avanti) o memorizzato nel "repository"
%  Modo  (+) semidet  (fallisce se non c'� una mappa con quel nome)
%
pred(existing_world(any)).
%  existing_world(Name) :  Name � definito da una mappa
...
%  dimensione Size
%  MODO (++,++) semidet
%  Errore se W non � stato caricato
%
...
% fact(W, F) : il fatto F � una propriet� del mondo W da memorizzare su
...
%  contenuto Cnt � rappresentato da Fig, che viene iscritta in un
